...
const bootstrap = `
  <html data-theme="default">
    <head>
      ...
      <script type="module">
        import { defineEpaperElements } from "/dist/wc/index.js";
        defineEpaperElements();
      </script>
    </head>
    <body>
      <epaper-button id="btn" variant="primary" size="sm">OK</epaper-button>
      <epaper-button id="btn-aria" aria-label="Close dialog" aria-expanded="true">X</epaper-button>
      <epaper-input id="input" type="email" aria-invalid="true" placeholder="mail"></epaper-input>
      <epaper-input id="input-aria" type="text" aria-describedby="help1" aria-label="Full name"></epaper-input>
      <epaper-checkbox id="cb" checked>Accept</epaper-checkbox>
      <epaper-grid id="grid" min="18rem">
        <div>One</div><div>Two</div>
      </epaper-grid>
      <epaper-alert id="alert-info" variant="info">Info message</epaper-alert>
      <epaper-alert id="alert-error" variant="error">Error message</epaper-alert>
      <epaper-divider id="divider"></epaper-divider>
    </body>
  </html>
`;
...
test.describe("Web Components (light DOM)", () => {
  ...
  test("button maps attributes to classes and disabled reflects", async ({ page }) => {
    ...
    await expect(btn).toHaveClass(/epaper-btn/);
    await expect(btn).toHaveClass(/epaper-btn--primary/);
    await expect(btn).toHaveClass(/epaper-btn--sm/);

    await page
      .locator("#btn")
      .evaluate((el) => el.setAttribute("variant", "secondary"));
    await expect(btn).toHaveClass(/epaper-btn--secondary/);
  ...
  test("style vars propagate (grid min column width)", async ({ page }) => {
    ...
    const styleVal = await grid.evaluate((el) =>
      getComputedStyle(el).getPropertyValue("--epaper-grid-min").trim()
    );
