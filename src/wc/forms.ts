...
export class EpaperInputElement extends BaseInputElement {
...
export class EpaperTextareaElement extends BaseInputElement {
  protected createControl(): HTMLTextAreaElement {
    return document.createElement("textarea");
  }
  protected baseClass() {
    return "epaper-textarea";
  }
}

export class EpaperSelectElement extends BaseInputElement {
  ...
  protected baseClass() {
    return "epaper-select";
  }
}

export class EpaperCheckboxElement extends HTMLElement {
  static get observedAttributes() {
    return [
      "checked",
      "disabled",
  ...
  attributeChangedCallback() {
    ...
    if (!this.#input) {
      const input = document.createElement("input");
      input.type = "checkbox";
      input.classList.add("epaper-checkbox");
...
export class EpaperRadioElement extends HTMLElement {
  static get observedAttributes() {
    return [
      "checked",
      "disabled",
  ...
  attributeChangedCallback() {
    ...
    if (!this.#input) {
      const input = document.createElement("input");
      input.type = "radio";
      input.classList.add("epaper-radio");
      input.addEventListener("change", () => {
        this.checked = input.checked;
      });
      this.insertBefore(input, this.firstChild);
...
export const defineFormComponents = () => {
  defineIfNeeded("epaper-input", EpaperInputElement);
  defineIfNeeded("epaper-textarea", EpaperTextareaElement);
  defineIfNeeded("epaper-select", EpaperSelectElement);
  defineIfNeeded("epaper-checkbox", EpaperCheckboxElement);
  defineIfNeeded("epaper-radio", EpaperRadioElement);
};
