...
/** Remove any existing epaper-* classes then re-apply the desired ones, preserving third-party classes. */
export const syncClassList = (el: HTMLElement, next: string[]): void => {
  const keep = Array.from(el.classList).filter((cls) => !cls.startsWith(EPAPER_PREFIX));
