...
export const defineClassComponent = (config: ComponentConfig) => {
  ...
  class EinkClassComponent extends HTMLElement {
    ...
    #sync() {
      const keep = Array.from(this.classList).filter((c) => !c.startsWith(EINK_PREFIX));
  ...
  if (!customElements.get(config.tag)) {
    customElements.define(config.tag, EinkClassComponent);
  }
};
