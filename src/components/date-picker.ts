...
export class EDatePicker extends BaseFormControl {
  ...
  connectedCallback() {
    ...
    this._placeholderHtml = placeholder;
    const value = this.getAttribute('value') || '';
    this._value = value;
    this.internals.setFormValue(value);

    const today = new Date();
    const ini = parseYMD(value) || today;
    ...
    onGlobal(this, document, 'mousedown', (e) => {
      if (!this.contains(e.target as Node) && this._pop) {
  ...
  private _onClick = (e: Event): void => {
    ...
    if (target.closest('[data-trigger]')) {
      if (this._pop) {
        this._pop.hidden = !this._pop.hidden;
        if (!this._pop.hidden) this._focusInitialCell();
      }
      return;
    }
  ...
  private _onKeydown = (e: Event): void => {
    ...
    if (target.closest('[data-trigger]') && this._pop) {
      if (ke.key === 'ArrowDown' || ke.key === 'Enter' || ke.key === ' ') {
        ke.preventDefault();
        if (this._pop.hidden) this._pop.hidden = false;
    ...
    if (ke.key === 'ArrowLeft') {
      ...
      ke.preventDefault();
      const start = Math.max(0, idx - (idx % 7));
      focusables[Math.min(start, focusables.length - 1)]?.focus();
    } else if (ke.key === 'End') {
      ke.preventDefault();
      const startOfWeek = idx - (idx % 7);
      const endOfWeek = Math.min(startOfWeek + 6, focusables.length - 1);
      focusables[endOfWeek]?.focus();
  ...
  attributeChangedCallback(name: string, _old: string | null, val: string | null) {
    if (name !== 'value' || !this._wired) return;
