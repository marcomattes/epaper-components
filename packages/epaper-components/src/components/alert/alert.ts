...
/**
 * One glyph per severity. Before v2.0.0 the map borrowed unrelated icons —
 ...
 * inside it.
 *
 * @attr {'info'|'success'|'warning'|'error'} [variant='info'] - Severity preset; controls icon and border treatment. Since v2.0.0 the icons are `info`, `check`, `warning` and `error` — previously `doc`, `check`, `bell` and `close`.
