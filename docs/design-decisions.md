...
All component selectors are wrapped in `:where()`, giving them specificity 0-0-0. This means any user stylesheet rule (even a single class) overrides eink-ui without `!important`.
...
The core library ships CSS only; however, an optional `eink-ui.dialog.polyfill.js` is provided for environments lacking native `<dialog>`. The polyfill uses semantic data attributes (`data-dialog-target`, `data-dialog-close`) and avoids animations to stay E-Ink friendly.
