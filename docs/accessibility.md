...
eink-ui is designed for accessible E-Ink interfaces. This guide covers ARIA attributes, keyboard navigation, and E-Ink-specific considerations.
...
| **Dialog**                       | `aria-labelledby="<title-id>"`                                                    | Points to `.eink-dialog__title` element.                                                                                      |
...
Use the `required` attribute (forwarded by WCs) and `.eink-label--required` for visual indication:
...
eink-ui tabs use hidden radio inputs rather than `role="tablist"` / `role="tab"`. This is intentional:
