...
│   │   ├── epaper-components.tokens.scss          Primitives + 3 theme overrides
│   │   ├── epaper-components.base.scss            Reset + tag-level styles (:where wrapped)
│   │   ├── epaper-components.components.scss      Manifest — imports all partials
│   │   └── components/
...
│   │   ├── index.ts                     Entry: defineEpaperElements() registers all
│   │   ├── component-factory.ts         Factory: attribute → class mapping
│   │   ├── base.ts                      Shared utilities
│   │   ├── button.ts                    <epaper-button> (wraps native <button>)
...
├── dist/                                Built output (epaper-components.css + epaper-components.min.css)
...
demo/*.css           (3 individual files, also used as Storybook fallback)
  ↓  concatenate
dist/epaper-components.css     (full bundle, ~26 KB)
  ↓  clean-css level:2
dist/epaper-components.min.css (~20 KB)
  ↓  transpile
dist/epaper-components.dialog.polyfill.js (ESM, optional)
...
- All components use class selectors with the `.epaper-*` prefix
- Every selector is wrapped in `:where()` for zero specificity (0-0-0)
- **Dual selectors** include both class and element names: `:where(.epaper-card, eink-card)` — the same rule block styles both CSS-only and Web Component usage with zero duplication
- Modifier variants use attribute selectors for WC equivalents: `:where(.epaper-card--raised, eink-card[raised])`
...
```
.epaper-{component}              Base class
.epaper-{component}--{variant}   Variant modifier
.epaper-{component}__{child}     Child element
.epaper-{component}--{size}      Size modifier (sm, lg)
```

Examples: `.epaper-btn--primary`, `.epaper-card__footer`, `.epaper-table--striped`
...
| --------- | ---------------------------------- | -------------------------- |
| `boolean` | `<epaper-card raised>`             | Adds `eink-card--raised`   |
| `enum`    | `<epaper-alert variant="warning">` | Adds `eink-alert--warning` |
...
:where(.epaper-container, eink-container) {
  display: block;
  width: 100%;
  max-width: 52rem;
  margin-inline: auto;
...
:where(.epaper-container--wide, eink-container[width="wide"]) {
  max-width: 72rem;
}
```
