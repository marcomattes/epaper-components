...
- An open GitHub PR: use the `mcp__github__pull_request_read` /
  `get_file_contents` tools to fetch the diff and file contents rather than
  guessing from the PR description.
- Read full files around each hunk, not just the patch context — several of
  the checks below (missing `onGlobal` cleanup, missing JSDoc block) are
  easy to miss from a 3-line diff.
...
- For a local/self-review, report findings inline as text, most-severe
  first (correctness/leak/security issues before style).
- For an actual GitHub PR, use `pull_request_review_write` (method
  ...
  then `submit_pending` — per the repo's own PR-driving rules, only push
  fixes for small/local asks yourself; larger design-level findings go in
  the review as a comment, not as an unrequested refactor.
