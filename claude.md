...
- Act as a senior design-system engineer for E‑Ink UIs.
...
2. Define tokens in `eink-ui.tokens.css`.
3. Define reset/base in `eink-ui.base.css`.
4. Implement components in `eink-ui.components.css`.
...
- Class-based `.eink-*` prefix with `:where()` for zero specificity.
- CSS selectors include **dual targets**: `:where(.eink-card, eink-card)` — the class for CSS-only usage, the element name for Web Components. One rule block, zero duplication.
- Modifier variants use attribute selectors for WC: `:where(.eink-card--raised, eink-card[raised])`.
...
- **Form wrappers:** `src/wc/forms.ts` — `<eink-input>`, `<eink-select>`, etc. wrap native controls (no Shadow DOM).
- **Button:** `src/wc/button.ts` — wraps native `<button>` with variant/size/disabled attributes.
- **Entry point:** `src/wc/index.ts` — `defineEinkElements()` registers all custom elements.
