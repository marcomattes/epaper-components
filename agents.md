...
2. Create `eink-ui.tokens.css` for tokens.
3. Create `eink-ui.base.css` for reset/base primitives.
4. Create `eink-ui.components.css` for all components.
...
- Choose exactly one: class-based `.eink-ui-*` or attribute-based `[data-ui="button"]`. Stay consistent. Justify choice briefly in docs.
