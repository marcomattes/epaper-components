...
Working guide for Claude Code (and other LLM agents) in this repository.
Human contributors should read `CONTRIBUTING.md` instead — this file
duplicates only what's necessary for an agent to be useful without
re-discovering the project structure each session.

## TL;DR

This is **EPaper**, a vanilla web component library (59 source files, 82 registered Custom Elements)
...
```
src/
  components/         59 components, one file each, ending with `define('e-tag', Class)`
...
than in this file. CLAUDE.md is for stable conventions; memories are for
in-flight context.
